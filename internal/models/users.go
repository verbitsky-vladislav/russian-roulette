...
type User struct {
	...
	TotalWins     null.Int          `boil:"total_wins" json:"total_wins,omitempty" toml:"total_wins" yaml:"total_wins,omitempty"`
	TotalLosses   null.Int          `boil:"total_losses" json:"total_losses,omitempty" toml:"total_losses" yaml:"total_losses,omitempty"`
...
var UserWhere = struct {
	...
	TotalWins     whereHelpernull_Int
	TotalLosses   whereHelpernull_Int
	...
	TotalWins:     whereHelpernull_Int{field: "\"users\".\"total_wins\""},
	TotalLosses:   whereHelpernull_Int{field: "\"users\".\"total_losses\""},
...
type userR struct {
	CreatorGames GameSlice       `boil:"CreatorGames" json:"CreatorGames" toml:"CreatorGames" yaml:"CreatorGames"`
...
func (r *userR) GetCreatorGames() GameSlice {
	if r == nil {
		return nil
	}
	return r.CreatorGames
}

func (r *userR) GetGamePlayers() GamePlayerSlice {
	if r == nil {
		return nil
	}
...
func (r *userR) GetGameRounds() GameRoundSlice {
	if r == nil {
		return nil
	}
	return r.GameRounds
...
var (
	userAllColumns            = []string{"uuid", "chat_id", "tg_name", "balance", "wallet_address", "total_wins", "total_losses", "created_at"}
	userColumnsWithoutDefault = []string{"chat_id", "tg_name"}
	userColumnsWithDefault    = []string{"uuid", "balance", "wallet_address", "total_wins", "total_losses", "created_at"}
	userPrimaryKeyColumns     = []string{"uuid"}
...
// CreatorGames retrieves all the game's Games with an executor via creator_uuid column.
func (o *User) CreatorGames(mods ...qm.QueryMod) gameQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"game\".\"creator_uuid\"=?", o.UUID),
	)
...
func (o *User) GamePlayers(mods ...qm.QueryMod) gamePlayerQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"game_players\".\"user_uuid\"=?", o.UUID),
	)
...
func (o *User) GameRounds(mods ...qm.QueryMod) gameRoundQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"game_rounds\".\"user_uuid\"=?", o.UUID),
	)
...
func (userL) LoadCreatorGames(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`game`),
		qm.WhereIn(`game.creator_uuid in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.UUID, foreign.CreatorUUID) {
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadGamePlayers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`game_players.user_uuid in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadGameRounds(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`game_rounds`),
		qm.WhereIn(`game_rounds.user_uuid in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *User) AddCreatorGames(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Game) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.CreatorUUID, o.UUID)
...
// SetCreatorGames removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Creator's CreatorGames accordingly.
// Replaces o.R.CreatorGames with related.
// Sets related.R.Creator's CreatorGames accordingly.
func (o *User) SetCreatorGames(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Game) error {
	query := "update \"game\" set \"creator_uuid\" = null where \"creator_uuid\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.CreatorGames {
			queries.SetScanner(&rel.CreatorUUID, nil)
			if rel.R == nil {
				continue
			}
		...
		o.R.CreatorGames = nil
	}

	return o.AddCreatorGames(ctx, exec, insert, related...)
}

// RemoveCreatorGames relationships from objects passed in.
// Removes related items from R.CreatorGames (uses pointer comparison, removal does not keep order)
// Sets related.R.Creator.
func (o *User) RemoveCreatorGames(ctx context.Context, exec boil.ContextExecutor, related ...*Game) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("creator_uuid")); err != nil {
			return err
		}
	}
	if o.R == nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.CreatorGames {
			if rel != ri {
				continue
			}

			ln := len(o.R.CreatorGames)
			if ln > 1 && i < ln-1 {
				o.R.CreatorGames[i] = o.R.CreatorGames[ln-1]
			}
			o.R.CreatorGames = o.R.CreatorGames[:ln-1]
			break
		}
	}
...
func (o *User) AddGamePlayers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*GamePlayer) error {
	var err error
	for _, rel := range related {
		if insert {
...
// Sets related.R.User appropriately.
func (o *User) AddGameRounds(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*GameRound) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.UserUUID, o.UUID)
...
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's GameRounds accordingly.
// Replaces o.R.GameRounds with related.
// Sets related.R.User's GameRounds accordingly.
func (o *User) SetGameRounds(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*GameRound) error {
	query := "update \"game_rounds\" set \"user_uuid\" = null where \"user_uuid\" = $1"
	values := []interface{}{o.UUID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
...
// Sets related.R.User.
func (o *User) RemoveGameRounds(ctx context.Context, exec boil.ContextExecutor, related ...*GameRound) error {
	if len(related) == 0 {
		return nil
	}
...
func FindUser(ctx context.Context, exec boil.ContextExecutor, uUID string, selectCols ...string) (*User, error) {
	userObj := &User{}
	...
	query := fmt.Sprintf(
		"select %s from \"users\" where \"uuid\"=$1", sel,
	)
...
func (o *User) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindUser(ctx, exec, o.UUID)
	if err != nil {
		return err
	}
...
func UserExists(ctx context.Context, exec boil.ContextExecutor, uUID string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"users\" where \"uuid\"=$1 limit 1)"
...
func (o *User) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return UserExists(ctx, exec, o.UUID)
}
